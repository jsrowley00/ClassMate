...
*   **Stripe Integration**: Payment processing via Stripe Checkout and `stripe-replit-sync` for product/price sync and webhooks. One-time $40 payment for 4-month student access with expiration tracking. Users table includes `stripeCustomerId`, `stripePaymentId`, `subscriptionStatus`, and `subscriptionExpiresAt` fields. Access expiration enforced via `checkStudentAccess` middleware on content-creation endpoints (generating practice tests, flashcards, AI chats). Expired students can still view their past content (chat history, flashcard sets, practice test results) but cannot create new content.